...
struct NetworkManager {
    ...
    private let router = Router<CreditScoreApi>()
    ...
    func getCreditScore(completion: @escaping (_ creditScore: CreditScore?, _ error: String?)->()) {
        router.request(.creditScore) { (data, response, error) in
            if error != nil {
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case .success:
                    ...
                    do {
                        let apiResponse = try JSONDecoder().decode(CreditScore.self, from: responseData)
